/**
 * Playwright E2E test for My Ghost Writer "Text Stats" feature on iPad Mini (Classic UI).
 *
 * This test case mirrors the desktop -3- test, but runs in iPad Mini viewport.
 * It covers:
...
test('test My Ghost Writer, iPad Mini: try using My Ghost Writer with Aesthetic UI then with Corpo UI', async ({ page }: { page: Page }) => {
  ...
  try {
    await expect(page.getByRole('button', { name: 'Show Corpo Side Panel' })).toBeVisible({timeout: 200});
    await page.getByRole('button', { name: 'Show Corpo Side Panel' }).click({timeout: 200});
  } catch {
    console.log("side menu already visible?")
  }
  await page.getByText('Raw Editor', { exact: true }).click();
