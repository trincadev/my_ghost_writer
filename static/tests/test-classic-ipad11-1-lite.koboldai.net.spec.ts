...
const test = base.extend({
  page: async ({ page }, use) => {
    await page.context().newPage(iPad11);
    await use(page);
  },
});
...
test('test My Ghost Writer, iPad 11: order/sort', async ({ page }) => {
  ...
  for (let currentOrderSelectionValueIdx in orderSelectionValues) {
    ...
    for (let currentSortSelectionValueIdx in sortSelectionValues) {
      ...
      await page.getByRole('searchbox', { name: 'filter-words-frequency' }).press('Enter');
      await page.waitForTimeout(300)
      await expect(page.getByLabel('id-list-of-words-container')).toMatchAriaSnapshot({ name: `test-classic-ipad11-1--${currentOrderSelectionValue}-${currentSortSelectionValue}.txt` });
    }
  }
  console.log("end!")
  page.close()
