...
test('test-classic-mobile-pixel7-1-lite: My Ghost Writer, Pixel 7: menu opens/closes on filter and sort/order', async ({ page }) => {
  // Go to the app
  ...
  for (let currentOrderSelectionValueIdx in orderSelectionValues) {
    ...
    for (let currentSortSelectionValueIdx in sortSelectionValues) {
      ...
      await expect(page.getByLabel('id-list-of-words-container')).toMatchAriaSnapshot({ name: `test-classic-mobile-pixel7-1--${currentOrderSelectionValue}-${currentSortSelectionValue}.txt` });
    }
  }
  // End of test
  console.log("end!");
